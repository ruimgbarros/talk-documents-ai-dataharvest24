<script>
	...
	async function debugHandleSubmit(event) {
		...
		const response = await fetch('/chat/api/embeddings', {
			method: 'POST',
	...
	<div class="bg-neutral-950 hidden md:flex flex-col gap-2 text-white">
		...
		<div class="overflow-auto flex-1">
			<div class="grid gap-1 p-2">
				...
				<a
					href="./embeddings"
					class="truncate overflow-hidden flex-1 text-sm transition-colors rounded-md whitespace-nowrap p-2 block bg-neutral-400 hover:bg-neutral-600 hover:text-neutral-50"
				>
		...
		<div class="max-w-2xl flex-1 mx-auto flex flex-col items-start gap-8 px-4 w-full">
			<div class="flex items-start gap-4">
				<div class="grid gap-1">
					{#if embed}
						<div class="font-bold">Embedding</div>
						<div class="prose prose-stone">
							<p>{JSON.stringify(embed)}</p>
						</div>
					{/if}
				</div>
			</div>
